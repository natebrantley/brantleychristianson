...
- **MailerLite** (push leads to list): `GET /api/cron/sync-leads-to-mailerlite` uses `MAILERLITE_API_TOKEN` + optional `MAILERLITE_GROUP_ID`; protect with `CRON_SECRET` (see docs/WEBHOOK-MAILERLITE.md). Sync uses MailerLite batch API and incremental window by default. **Automated:** Vercel Cron runs every hour (only leads updated in last 2h are synced per run to avoid timeouts/rate limits); run manually without `?since=` for a full sync.
