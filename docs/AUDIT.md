...
- **Auth & Clerk** – `middleware.ts` (Clerk), `auth()` / `getToken()` in server components, role logic (`isBrokerRole`), redirects.
...
| Area | Status | Notes |
|------|--------|--------|
| Middleware | OK | Clerk runs via `middleware.ts`; matcher excludes static assets. |
