      ...
      <section className="section broker-detail-section" aria-labelledby="broker-name-heading">
        <div className="container container-narrow stack--lg">
          <header className="stack--sm text-center">
            <div className="broker-detail-photo-wrap">
              <Image
                src={agent.image}
          ...
          <article className="stack--lg">
            <div className="broker-detail-actions">
              ...
              {agent.phone ? (
                <Button href={`tel:${agent.phone.replace(/\D/g, '')}`} variant="outline">
                  Call {agent.phone}
                </Button>
              ) : null}
              {!assignCtaProminent && (
            ...
            {assignCtaProminent && (
              <div className="agent-assign-cta" style={{ background: 'var(--color-light-alt)', border: '1px solid rgba(var(--color-accent-rgb), 0.2)', borderRadius: 'var(--radius-md)', padding: '1rem 1.25rem', textAlign: 'center' }} aria-label="Assign this agent">
                <p style={{ margin: '0 0 0.75rem 0', fontWeight: 600 }}>Want this agent as your main contact?</p>
                <p style={{ margin: 0, fontSize: '0.9375rem', color: 'var(--color-text-muted)' }}>Choose them below to see their info and call button on your dashboard.</p>
            ...
            {languageCta ? (
              <div className="broker-detail-language-cta">
                <p className="broker-detail-language-cta-text">{languageCta.description}</p>
                <Button href={`mailto:${agent.email}`} variant="outline">{languageCta.label}</Button>
              </div>
            ) : null}
          </article>
        </div>
